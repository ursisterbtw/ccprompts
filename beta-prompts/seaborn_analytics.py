...
class SeabornAnalytics:
    ...
    def create_comprehensive_analytics_suite(self, data: Optional[pd.DataFrame] = None) -> str:
        """
        ...
        fig.suptitle('🔬 ADVANCED PROMPT OPTIMIZATION ANALYTICS LABORATORY 🔬', 
                    fontsize=32, fontweight='bold', color=SEABORN_DARK_THEME['primary'],
                    y=0.98)
    ...
    def _create_correlation_matrix(self, ax, data: pd.DataFrame):
        ...
        numeric_cols = ['quality_score', 'improvement_rate', 'complexity_score', 
                       'implementation_effort', 'usage_frequency', 'roi_score']
        ...
        cbar.set_label('Correlation Coefficient', color=SEABORN_DARK_THEME['text_primary'], 
                      fontweight='bold', fontsize=12)
        
        # Styling
        ax.set_title('🔗 FEATURE CORRELATION MATRIX', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ...
        ax.text(0.02, 0.98, f'💪 Strongest Correlation:\n{strongest_pair[0]} ↔ {strongest_pair[1]}\n{strongest_value:.3f}',
               transform=ax.transAxes, fontsize=10, fontweight='bold',
               color=SEABORN_DARK_THEME['neon_lime'], verticalalignment='top',
               bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], alpha=0.8))
    ...
    def _create_performance_distributions(self, ax, data: pd.DataFrame):
        ...
        for i, (category, color) in enumerate(zip(categories, colors)):
            category_data = data[data['technique_category'] == category]['quality_score']
            ...
            # Add text annotation
            ax.text(mean_val, ax.get_ylim()[1] * 0.9 - i * 0.15, 
                   f'{category}\nμ={mean_val:.2f}, σ={std_val:.2f}',
                   ha='center', va='top', fontweight='bold', color=color,
                   bbox=dict(boxstyle='round,pad=0.3', facecolor=SEABORN_DARK_THEME['card'], 
        ...
        ax.text(0.02, 0.98, f'📊 Distribution Analysis\nSkewness: {overall_skew:.2f}\nKurtosis: {overall_kurtosis:.2f}',
               transform=ax.transAxes, fontsize=11, fontweight='bold',
               color=SEABORN_DARK_THEME['text_primary'], verticalalignment='top',
               bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], alpha=0.8))
        
        ax.set_title('📈 QUALITY SCORE DISTRIBUTIONS BY TECHNIQUE', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ax.set_xlabel('Quality Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel('Density', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
    ...
    def _create_technique_swarm_plot(self, ax, data: pd.DataFrame):
        ...
        for i, category in enumerate(categories):
            category_data = data[data['technique_category'] == category]['improvement_rate']
            ...
            ax.hlines(median_val, i-0.4, i+0.4, colors=SEABORN_DARK_THEME['neon_lime'], 
                     linewidth=4, alpha=0.9, zorder=10)
            
            # Add IQR annotation
            ax.text(i, ax.get_ylim()[1] * 0.95, f'IQR: {iqr:.2f}',
                   ha='center', va='top', fontweight='bold', 
                   color=SEABORN_DARK_THEME['text_primary'], fontsize=10,
                   bbox=dict(boxstyle='round,pad=0.2', facecolor=self.technique_palette[i], alpha=0.7))
        
        ax.set_title('🎯 TECHNIQUE EFFECTIVENESS DISTRIBUTION', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ax.set_xlabel('Optimization Technique', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel('Improvement Rate', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
    ...
    def _create_domain_violin_plot(self, ax, data: pd.DataFrame):
        ...
        # Add zone labels
        ax.text(ax.get_xlim()[1] * 0.95, 0.9, 'EXCELLENT', ha='right', va='center',
               fontweight='bold', color=SEABORN_DARK_THEME['success'], fontsize=12)
        ax.text(ax.get_xlim()[1] * 0.95, 0.7, 'GOOD', ha='right', va='center',
               fontweight='bold', color=SEABORN_DARK_THEME['warning'], fontsize=12)
        ax.text(ax.get_xlim()[1] * 0.95, 0.5, 'NEEDS WORK', ha='right', va='center',
               fontweight='bold', color=SEABORN_DARK_THEME['error'], fontsize=12)
        
        ax.set_title('🏭 DOMAIN PERFORMANCE BY COMPLEXITY', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ax.set_xlabel('Application Domain', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel('Quality Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        
        # Customize legend
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, title='Complexity Level', 
    ...
    def _create_optimization_timeline(self, ax, data: pd.DataFrame):
        ...
        slope, intercept, r_value, p_value, std_err = linregress(x_numeric, data_sorted['quality_score'])
        
        # Add trend information
        trend_text = f'📈 Trend Analysis\nSlope: {slope:.4f}\nR²: {r_value**2:.3f}\np-value: {p_value:.3e}'
        ax.text(0.02, 0.98, trend_text, transform=ax.transAxes, fontsize=11, fontweight='bold',
               color=SEABORN_DARK_THEME['neon_lime'], verticalalignment='top',
               bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], alpha=0.9))
        
        # Add moving average
        window_size = max(5, len(data_sorted) // 10)
        moving_avg = data_sorted['quality_score'].rolling(window=window_size, center=True).mean()
        ax.plot(data_sorted['optimization_date'], moving_avg, 
               color=SEABORN_DARK_THEME['gold'], linewidth=3, alpha=0.8, 
               label=f'Moving Average ({window_size})')
        
        ax.set_title('⏱️ OPTIMIZATION PROGRESS TIMELINE', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ax.set_xlabel('Optimization Date', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel('Quality Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ...
        # Customize legend
        handles, labels = ax.get_legend_handles_labels()
        ax.legend(handles, labels, title='Technique & Size=Improvement', 
                 title_fontsize=10, fontsize=9, loc='upper left',
                 facecolor=SEABORN_DARK_THEME['card'], 
                 edgecolor=SEABORN_DARK_THEME['grid'])
    ...
    def _create_feature_importance(self, ax, data: pd.DataFrame):
        ...
        features = ['complexity_score', 'implementation_effort', 'usage_frequency', 
                   'technique_effectiveness', 'domain_familiarity']
        ...
        colors = [SEABORN_DARK_THEME['success'] if sig else SEABORN_DARK_THEME['warning'] 
        ...
        # Add value labels
        for i, (bar, importance, p_val) in enumerate(zip(bars, importance_df['importance'], importance_df['p_value'])):
            ax.text(importance + 0.01, i, f'{importance:.3f}',
                   va='center', fontweight='bold', color=SEABORN_DARK_THEME['text_primary'])
            ...
            ax.text(importance + 0.05, i, sig_text,
                   va='center', fontweight='bold', color=SEABORN_DARK_THEME['neon_lime'])
        
        # Customize axes
        ax.set_yticks(range(len(importance_df)))
        ax.set_yticklabels(importance_df['feature'])
        ax.set_xlabel('Feature Importance (|Correlation|)', fontweight='bold', 
                     color=SEABORN_DARK_THEME['text_secondary'])
        
        ax.set_title('🔍 FEATURE IMPORTANCE ANALYSIS', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        
        # Add significance legend
        legend_text = 'Significance:\n*** p<0.001\n** p<0.01\n* p<0.05\nns p≥0.05'
        ax.text(0.98, 0.02, legend_text, transform=ax.transAxes, fontsize=10, fontweight='bold',
               color=SEABORN_DARK_THEME['text_primary'], verticalalignment='bottom', horizontalalignment='right',
               bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], alpha=0.9))
    ...
    def _create_clustering_analysis(self, ax, data: pd.DataFrame):
        ...
        features = ['quality_score', 'improvement_rate', 'complexity_score', 
                   'implementation_effort', 'usage_frequency']
        ...
        for i, center in enumerate(centers_pca):
            ax.annotate(f'C{i+1}', center, fontsize=12, fontweight='bold',
                       ...
                       bbox=dict(boxstyle='circle,pad=0.3', facecolor=SEABORN_DARK_THEME['neon_lime'], alpha=0.8))
        ...
        ax.text(0.02, 0.98, variance_text, transform=ax.transAxes, fontsize=11, fontweight='bold',
               color=SEABORN_DARK_THEME['text_primary'], verticalalignment='top',
               bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], alpha=0.9))
        
        ax.set_title('🎯 OPTIMIZATION PATTERN CLUSTERING', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ax.set_xlabel(f'First Principal Component ({pca.explained_variance_ratio_[0]:.1%})', 
                     fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel(f'Second Principal Component ({pca.explained_variance_ratio_[1]:.1%})', 
                     fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        
        # Add colorbar
        cbar = plt.colorbar(scatter, ax=ax, shrink=0.8)
        cbar.set_label('Cluster', color=SEABORN_DARK_THEME['text_primary'], fontweight='bold')
    ...
    def _create_prediction_model_viz(self, ax, data: pd.DataFrame):
        ...
        ax.plot([min_val, max_val], [min_val, max_val], 
               color=SEABORN_DARK_THEME['neon_lime'], linewidth=4, alpha=0.8, linestyle='--',
        ...
        ax.fill_between([min_val, max_val], 
                       ...
                       [min_val + margin, max_val + margin],
                       alpha=0.2, color=SEABORN_DARK_THEME['success'])
        
        # Add model performance metrics
        metrics_text = f'🤖 Model Performance\nR² Score: {r2:.3f}\nRMSE: {rmse:.3f}\nSamples: {len(y_test)}'
        ax.text(0.02, 0.98, metrics_text, transform=ax.transAxes, fontsize=11, fontweight='bold',
               color=SEABORN_DARK_THEME['text_primary'], verticalalignment='top',
               bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], alpha=0.9))
        
        ax.set_title('🔮 PREDICTIVE MODEL PERFORMANCE', fontsize=18, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=20)
        ax.set_xlabel('Actual Quality Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel('Predicted Quality Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        
        # Add colorbar for prediction error
        cbar = plt.colorbar(scatter, ax=ax, shrink=0.8)
        cbar.set_label('Prediction Error', color=SEABORN_DARK_THEME['text_primary'], fontweight='bold')
        cbar.ax.tick_params(colors=SEABORN_DARK_THEME['text_primary'])
    ...
    def _add_analytical_decorations(self, fig):
        ...
        fig.text(0.99, 0.01, f'⏰ Generated: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}',
                ha='right', va='bottom', fontsize=10, color=SEABORN_DARK_THEME['text_secondary'],
                bbox=dict(boxstyle='round,pad=0.3', facecolor=SEABORN_DARK_THEME['card'], alpha=0.8))
        
        # Add performance indicator
        fig.text(0.01, 0.99, '🔬 STATISTICAL ANALYSIS ACTIVE', 
                ha='left', va='top', fontsize=14, fontweight='bold', color=SEABORN_DARK_THEME['success'],
                bbox=dict(boxstyle='round,pad=0.5', facecolor=SEABORN_DARK_THEME['card'], 
                         edgecolor=SEABORN_DARK_THEME['success'], linewidth=2, alpha=0.9))
        ...
        for corner in [(0, 0), (1, 0), (0, 1), (1, 1)]:
            ...
            fig.add_artist(patches.Rectangle(
                (x - corner_size/2, y - corner_size/2), corner_size, corner_size,
                facecolor=SEABORN_DARK_THEME['primary'], alpha=0.6, transform=fig.transFigure
            ))
    ...
    def _generate_rich_dataset(self) -> pd.DataFrame:
        ...
        techniques = ['Role Definition', 'XML Structure', 'Chain of Thought', 
                     'Output Format', 'Examples', 'Structured Instructions']
        domains = ['Software Engineering', 'Data Science', 'Business Analysis', 'Marketing', 'Research']
        ...
        quality_base = (0.3 * data['technique_effectiveness'] + 
        ...
        data['improvement_rate'] = np.clip(data['quality_score'] * 0.4 + np.random.normal(0, 0.05, n_samples), 0, 1)
        data['roi_score'] = data['improvement_rate'] / (data['implementation_effort'] / 5)
        ...
        data['technique_effectiveness'] = np.clip(data['technique_effectiveness'], 0.3, 1)
        data['domain_familiarity'] = np.clip(data['domain_familiarity'], 0.2, 1)
        
        return pd.DataFrame(data)
    
    def create_publication_ready_plots(self, data: Optional[pd.DataFrame] = None) -> List[str]:
        """
        ...
        plt.savefig(filename1, dpi=300, bbox_inches='tight', facecolor=SEABORN_DARK_THEME['background'])
        plt.close()
        ...
        plt.savefig(filename2, dpi=300, bbox_inches='tight', facecolor=SEABORN_DARK_THEME['background'])
        plt.close()
        ...
        plt.savefig(filename3, dpi=300, bbox_inches='tight', facecolor=SEABORN_DARK_THEME['background'])
        plt.close()
    ...
    def _create_advanced_correlation_network(self, ax, data: pd.DataFrame):
        ...
        for (u, v, d) in G.edges(data=True):
            weight = d['weight']
            color = SEABORN_DARK_THEME['primary'] if weight > 0.7 else SEABORN_DARK_THEME['secondary']
            ax.plot([pos[u][0], pos[v][0]], [pos[u][1], pos[v][1]], 
                   color=color, alpha=weight, linewidth=weight*10)
        
        # Nodes
        for node in G.nodes():
            x, y = pos[node]
            ax.scatter(x, y, s=1000, c=SEABORN_DARK_THEME['neon_lime'], 
                      alpha=0.8, edgecolor='white', linewidth=3, zorder=10)
            ax.text(x, y, node.replace('_', '\n'), ha='center', va='center',
                   fontweight='bold', color=SEABORN_DARK_THEME['background'], fontsize=9)
        
        ax.set_title('🕸️ FEATURE CORRELATION NETWORK', fontsize=20, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=30)
        ax.axis('off')
    ...
    def _create_performance_evolution_landscape(self, ax, data: pd.DataFrame):
        ...
        data_sorted = data.sort_values('optimization_date')
        data_sorted['days'] = (data_sorted['optimization_date'] - data_sorted['optimization_date'].min()).dt.days
        
        # Create performance surface
        from scipy.interpolate import griddata
        
        # Grid for interpolation
        days_grid = np.linspace(data_sorted['days'].min(), data_sorted['days'].max(), 50)
        ...
        ax.plot(moving_avg.index, moving_avg.values, 
               color=SEABORN_DARK_THEME['neon_lime'], linewidth=5, alpha=0.9,
               label='Performance Trajectory')
        
        ax.set_title('🌊 PERFORMANCE EVOLUTION LANDSCAPE', fontsize=20, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=30)
        ax.set_xlabel('Days Since Start', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        ax.set_ylabel('Quality Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
    ...
    def _create_multidimensional_quality_analysis(self, ax, data: pd.DataFrame):
        ...
        features = ['quality_score', 'improvement_rate', 'technique_effectiveness', 'domain_familiarity']
        plot_data = data[features + ['technique_category']].copy()
        
        # Normalize features to 0-1 scale
        for feature in features:
            plot_data[feature] = (plot_data[feature] - plot_data[feature].min()) / (plot_data[feature].max() - plot_data[feature].min())
        ...
        ax.set_title('🌈 MULTI-DIMENSIONAL QUALITY ANALYSIS', fontsize=20, fontweight='bold',
                    color=SEABORN_DARK_THEME['text_primary'], pad=30)
        ax.set_ylabel('Normalized Score', fontweight='bold', color=SEABORN_DARK_THEME['text_secondary'])
        
        # Customize legend
        ax.legend(title='Optimization Technique', bbox_to_anchor=(1.05, 1), loc='upper left')
