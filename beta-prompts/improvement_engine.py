...
{technical_input}'''
    ...
    def analyze_prompt(self, prompt: str) -> PromptAnalysis:
        ...
        has_structured_instructions = bool(re.search(r'<instructions>|<steps>|1\.|2\.|3\.', prompt, re.IGNORECASE))
        has_xml_tags = len(re.findall(r'<\w+>', prompt)) > 0
        has_examples = bool(re.search(r'<example>|for example|e\.g\.|example:', prompt, re.IGNORECASE))
        has_thinking_process = bool(re.search(r'<thinking>|think step by step|let me think', prompt, re.IGNORECASE))
        has_output_format = bool(re.search(r'<output_format>|format:|output:', prompt, re.IGNORECASE))
        ...
        clarity_indicators = [
            ...
            bool(re.search(r'please|should|must|need', prompt, re.IGNORECASE))  # Has clear directives
        ]
    ...
    def optimize_prompt(self, prompt: str, task_type: str = 'general', 
                       domain: str = 'general', enhance_reasoning: bool = True) -> OptimizationResult:
        """
        ...
        if not analysis.has_examples and task_type in ['analysis', 'creative', 'technical']:
...
4. Finally, I'll review and refine the output"""
    ...
    def _insert_thinking_section(self, prompt: str, thinking_content: str) -> str:
        ...
        if '<instructions>' in prompt:
            return prompt.replace('</instructions>', f'</instructions>\n\n<thinking>\n{thinking_content}\n</thinking>')
        elif '<role>' in prompt:
            return prompt.replace('</role>', f'</role>\n\n<thinking>\n{thinking_content}\n</thinking>')
        else:
            return f"<thinking>\n{thinking_content}\n</thinking>\n\n{prompt}"
...
Rationale: [Why this example demonstrates the expected approach]
    ...
    def _insert_examples(self, prompt: str, example_content: str) -> str:
        ...
        if '<output_format>' in prompt:
            return prompt.replace('<output_format>', f'{example_content}\n\n<output_format>')
        else:
            return f"{prompt}\n\n{example_content}"
    ...
    def _predict_performance_improvement(self, analysis: PromptAnalysis, 
                                       improvements: List[str]) -> Dict[str, float]:
        ...
        for improvement in improvements:
            if improvement in improvement_factors:
                predicted_improvements[improvement] = improvement_factors[improvement]
    ...
    def batch_optimize_prompts(self, prompts: List[Dict[str, str]]) -> List[OptimizationResult]:
...
def main():
    ...
    for i, prompt_data in enumerate(test_prompts):
        ...
        print(f"Predicted Performance Improvement: {result.performance_prediction['overall_improvement']:.1%}")
        ...
        print("\n--- Optimized Prompt ---")
        print(result.optimized_prompt[:200] + "..." if len(result.optimized_prompt) > 200 else result.optimized_prompt)
