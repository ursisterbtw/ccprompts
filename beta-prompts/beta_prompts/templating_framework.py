...
class TemplateRegistry:
    ...
    def register_template(self, template_id: str, template_content: str, 
                         metadata: TemplateMetadata, variables: List[TemplateVariable]):
    ...
    def search_templates(self, task_type: Optional[str] = None, 
        ...
        matching_templates.sort(key=lambda tid: self.metadata_index[tid].quality_score, reverse=True)
...
class ContextEngine:
    ...
    def __init__(self):
        ...
        self.domain_contexts = {
            ...
            'data_science': {
                ...
                'techniques': ['Machine Learning', 'Statistical Analysis', 'Visualization'],
                'lifecycle': ['Data Collection', 'Exploration', 'Modeling', 'Validation']
            },
            'business_analysis': {
                'frameworks': ['SWOT', 'Porter\'s Five Forces', 'Value Chain', 'PEST'],
                'methodologies': ['Agile', 'Waterfall', 'Lean', 'Six Sigma'],
                'deliverables': ['Requirements', 'Process Maps', 'User Stories', 'Acceptance Criteria'],
                'stakeholders': ['Product Owner', 'Developers', 'End Users', 'Management']
            }
        }
    
    def adapt_template_to_context(self, template_content: str, context: Dict[str, Any]) -> str:
        """Adapt template based on context"""
        
        adapted_content = template_content
        
        for processor_name, processor_func in self.context_processors.items():
        ...
        if domain in self.domain_contexts:
            ...
            for key, values in domain_data.items():
                ...
                if placeholder in content:
                    content = content.replace(placeholder, values[0] if values else 'example')
    ...
    def _process_performance_context(self, content: str, context: Dict[str, Any]) -> str:
...
'''.strip()
        ...
        analysis_variables = [
            TemplateVariable('domain', 'string', 'Analysis domain (e.g., business, technical, financial)', True),
            TemplateVariable('experience', 'string', 'Years of experience', False, '10'),
            TemplateVariable('specialization', 'string', 'Area of specialization', False, 'comprehensive analysis'),
            TemplateVariable('task_description', 'string', 'Specific task description', True),
            TemplateVariable('analysis_steps', 'list', 'Analysis methodology steps', True),
            TemplateVariable('thinking_steps', 'list', 'Thinking process steps', True),
            TemplateVariable('output_sections', 'list', 'Output structure sections', True),
...
'''.strip()
        ...
        technical_variables = [
            TemplateVariable('role_type', 'string', 'Technical role type', False, 'software engineer'),
            TemplateVariable('experience', 'string', 'Years of experience', False, '10'),
            TemplateVariable('technology_stack', 'string', 'Primary technology stack', True),
            TemplateVariable('specialization', 'string', 'Area of technical specialization', False, 'scalable system design'),
            TemplateVariable('context_elements', 'list', 'Technical context elements', True),
            TemplateVariable('requirement_list', 'list', 'Technical requirements', True),
            TemplateVariable('constraint_list', 'list', 'Technical constraints', True),
            TemplateVariable('architecture_focus', 'string', 'Architecture analysis focus', False),
            TemplateVariable('implementation_strategy', 'string', 'Implementation approach', False),
            TemplateVariable('testing_strategy', 'string', 'Testing methodology', False),
            TemplateVariable('optimization_approach', 'string', 'Performance optimization strategy', False),
            TemplateVariable('output_elements', 'list', 'Output structure elements', True),
            TemplateVariable('code_examples', 'string', 'Code example content', False),
            TemplateVariable('technical_input', 'string', 'Technical input placeholder', False)
        ]
    ...
    def compile_template(self, template_id: str, variables: Dict[str, Any], 
                        context: Optional[Dict[str, Any]] = None) -> CompiledTemplate:
        ...
        if context:
            template_content = self.context_engine.adapt_template_to_context(template_content, context)
        
        # Validate variables
        template_variables = [TemplateVariable(**var_data) for var_data in template_data['variables']]
    ...
    def _validate_variables(self, template_variables: List[TemplateVariable], 
                           provided_variables: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for var in template_variables:
            if var.required and var.name not in provided_variables:
                ...
                if 'min_length' in var.constraints and len(str(value)) < var.constraints['min_length']:
                    raise ValueError(f"Variable '{var.name}' must be at least {var.constraints['min_length']} characters")
                if 'max_length' in var.constraints and len(str(value)) > var.constraints['max_length']:
                    raise ValueError(f"Variable '{var.name}' must be at most {var.constraints['max_length']} characters")
    ...
    def _predict_quality(self, compiled_prompt: str, metadata: TemplateMetadata) -> Dict[str, float]:
        ...
        structure_score = sum([has_role, has_instructions, has_thinking, has_output_format, has_examples]) / 5
        ...
        content_score = min(1.0, (word_count / 200) * 0.7 + (avg_sentence_length / 20) * 0.3)
        
        # Overall quality prediction
        overall_quality = (structure_score * 0.6 + content_score * 0.4) * metadata.quality_score
    ...
    def _estimate_performance(self, compiled_prompt: str, metadata: TemplateMetadata) -> Dict[str, float]:
        """Estimate performance characteristics"""
        
        # Token estimation (rough approximation)
        estimated_tokens = len(compiled_prompt.split()) * 1.3  # Account for tokenization
    ...
    def create_custom_template(self, name: str, content: str, variables: List[TemplateVariable],
                              task_types: List[str], domains: List[str], 
                              complexity_levels: List[str]) -> str:
        ...
        return self.registry.register_template(template_id, content, metadata, variables)
    ...
    def import_template(self, template_data: Union[str, Dict], format: str = 'json') -> str:
...
def main():
    ...
    compiled = framework.compile_template('comprehensive_analysis', analysis_variables, context)
    
    print(f"Compiled template (Quality: {compiled.quality_predictions['overall_quality']:.2f}):")
    print(compiled.compiled_prompt[:500] + "..." if len(compiled.compiled_prompt) > 500 else compiled.compiled_prompt)
...
'''.strip()
    ...
    custom_variables = [
        ...
        TemplateVariable('output_format', 'string', 'Output format specification', True),
    ...
    print(f"Estimated Processing Time: {compiled.performance_estimates['estimated_processing_time']:.2f}s")
    print(f"Estimated Input Tokens: {compiled.performance_estimates['estimated_input_tokens']:.0f}")
