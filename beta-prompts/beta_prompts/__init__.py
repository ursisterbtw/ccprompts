...
__description__ = (
    "Advanced prompt optimization research framework with real Claude API integration"
)
...
try:
    from .improvement_engine import OptimizationResult, PromptAnalysis, PromptImprovementEngine
...
try:
    from .generation_system import GeneratedPrompt, PromptBlueprint, PromptGenerationSystem
except ImportError as e:
    raise ImportError(
...
try:
    from .templating_framework import (
    ...
    raise ImportError(
        f"Failed to import templating_framework: {e}. " "Ensure Jinja2 is installed."
    ) from e
...
def get_info() -> dict[str, str | list[str]]:
    ...
    if _VISUALIZATION_EXPORTS:
        available_modules.extend(
            ["visualizations", "interactive_dashboard", "seaborn_analytics"]
        )
    if _SCORING_EXPORTS:
