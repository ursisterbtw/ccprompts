...
class PromptScorer:
    ...
    def calculate_efficiency_score(self) -> float:
        ...
        baseline_avg_time = sum(m.duration for m in self.baseline_metrics) / len(self.baseline_metrics)
        enhanced_avg_time = sum(m.duration for m in self.enhanced_metrics) / len(self.enhanced_metrics)
    ...
    def calculate_quality_score(self) -> float:
        ...
        baseline_success_rate = sum(1 for m in self.baseline_metrics if m.success) / len(self.baseline_metrics)
        enhanced_success_rate = sum(1 for m in self.enhanced_metrics if m.success) / len(self.enhanced_metrics)
        
        baseline_avg_errors = sum(m.error_count for m in self.baseline_metrics) / len(self.baseline_metrics)
        enhanced_avg_errors = sum(m.error_count for m in self.enhanced_metrics) / len(self.enhanced_metrics)
        
        success_improvement = (enhanced_success_rate - baseline_success_rate) * 100
        error_reduction = ((baseline_avg_errors - enhanced_avg_errors) / max(baseline_avg_errors, 1)) * 100
    ...
    def calculate_token_efficiency(self) -> float:
        ...
        ) / max(sum(1 for m in self.baseline_metrics if m.success), 1)
        
        enhanced_tokens_per_success = sum(
            m.tokens_used for m in self.enhanced_metrics if m.success
        ) / max(sum(1 for m in self.enhanced_metrics if m.success), 1)
        
        if enhanced_tokens_per_success == 0:
            return 100.0
            
        efficiency = ((baseline_tokens_per_success - enhanced_tokens_per_success) / baseline_tokens_per_success) * 100
    ...
    def calculate_iteration_improvement(self) -> float:
        ...
        baseline_avg_iterations = sum(m.iterations for m in self.baseline_metrics) / len(self.baseline_metrics)
        enhanced_avg_iterations = sum(m.iterations for m in self.enhanced_metrics) / len(self.enhanced_metrics)
        
        improvement = ((baseline_avg_iterations - enhanced_avg_iterations) / baseline_avg_iterations) * 100
    ...
    def get_detailed_comparison(self) -> Dict:
        ...
        baseline_stats = {
            ...
            'successful_tasks': sum(1 for m in self.baseline_metrics if m.success),
            'avg_duration': sum(m.duration for m in self.baseline_metrics) / len(self.baseline_metrics),
            'avg_iterations': sum(m.iterations for m in self.baseline_metrics) / len(self.baseline_metrics),
            'avg_tokens': sum(m.tokens_used for m in self.baseline_metrics) / len(self.baseline_metrics),
            'total_errors': sum(m.error_count for m in self.baseline_metrics)
        }
        
        enhanced_stats = {
            'total_tasks': len(self.enhanced_metrics),
            'successful_tasks': sum(1 for m in self.enhanced_metrics if m.success),
            'avg_duration': sum(m.duration for m in self.enhanced_metrics) / len(self.enhanced_metrics),
            'avg_iterations': sum(m.iterations for m in self.enhanced_metrics) / len(self.enhanced_metrics),
            'avg_tokens': sum(m.tokens_used for m in self.enhanced_metrics) / len(self.enhanced_metrics),
        ...
        return {
            ...
            'improvements': {
                'time_saved_per_task': baseline_stats['avg_duration'] - enhanced_stats['avg_duration'],
                'success_rate_increase': (enhanced_stats['successful_tasks'] / enhanced_stats['total_tasks']) - 
                                       (baseline_stats['successful_tasks'] / baseline_stats['total_tasks']),
                'iteration_reduction': baseline_stats['avg_iterations'] - enhanced_stats['avg_iterations'],
                'token_savings': baseline_stats['avg_tokens'] - enhanced_stats['avg_tokens'],
                'error_reduction': baseline_stats['total_errors'] - enhanced_stats['total_errors']
            },
